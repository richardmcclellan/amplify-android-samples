...
class ListActivity : AppCompatActivity() {
    private val itemAdapter: SimpleItemRecyclerViewAdapter = SimpleItemRecyclerViewAdapter(listOf(), setOf(), setOf(), this)
...
//            Where.sorted(Todo.NAME.ascending()),
        ...
        Amplify.Hub.subscribe(HubChannel.DATASTORE) {
            ...
            when (DataStoreChannelEventName.fromString(it.name)) {
                DataStoreChannelEventName.OUTBOX_MUTATION_PROCESSED -> {
                    pendingSaveIds.remove((it.data as OutboxMutationEvent<Todo>).element.model.id)
                    pendingDeleteIds.remove((it.data as OutboxMutationEvent<Todo>).element.model.id)
                    query()
                }
            }
        }
    ...
    class SimpleItemRecyclerViewAdapter(var values: List<Todo>,
        ...
        private val LOG = Amplify.Logging.forNamespace("app-datastore-todo:SimpleItemRecyclerViewAdapter")
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            if (pendingDeleteIds.contains(item.id)) {
                if(pendingSaveIds.contains(item.id)) {
    ...
    companion object {
        private val LOG = Amplify.Logging.forNamespace("app-datastore-todo:ListActivity")
    }
}
